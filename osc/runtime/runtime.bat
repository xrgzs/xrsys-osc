...
if %i% GEQ 2 (
    ...
    if exist DX9.exe (
        start "" /wait "%XRSYS_OSC_PECMD_EXE%" EXEC -wait -timeout:300000 DX9.exe /S
    )
    if exist DirectX_Redist_Repack_x86_x64_Final.exe (
        start "" /wait "%XRSYS_OSC_PECMD_EXE%" EXEC -wait -timeout:300000 DirectX_Redist_Repack_x86_x64_Final.exe /ai /gm2
    )
)
set i=0
...
@rem if %XRSYS_OSC_WINDOWS_VERSION_LEVEL% equ 1 ( set i=0 )
if %i% GEQ 2 (
...
if not exist "%SystemDrive%\Windows\Setup\xrsysnoruntime.txt" (
    ...
    ver | find /i "10.0." && (
        ...
        @rem if not exist "dotnet6-%os_arch%.exe" if "%isoffline%"=="0" %XRSYS_OSC_ARIA2_CMD% -x16 -j16 -s16 -o "dotnet6-%os_arch%.exe" "%XRSYS_OSC_LINK_BASE_URL%/dotnet6-%os_arch%"
